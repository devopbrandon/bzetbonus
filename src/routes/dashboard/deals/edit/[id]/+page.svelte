<script lang="ts">
	...
	let { data }: { data: { deal: Deal } } = $props();

	let deal = $derived(data.deal);

	let brand = $state(deal?.brand ?? '');
	let promocode = $state(deal?.promocode ?? '');
	let bonus = $state(deal?.bonus ?? '');

	let bonustype = $state<'non-sticky' | 'sticky' | 'wagerfree'>(
		(deal?.bonustype as 'non-sticky' | 'sticky' | 'wagerfree') ?? 'non-sticky'
	);

	let maxbonus = $state(deal?.maxbonus ?? '');
	let maxbet = $state(deal?.maxbet ?? '');
	let freespins = $state<string | number>(deal?.freespins ?? '');
	let wager = $state(deal?.wager ?? '');

	let wagertype = $state<'Wagerfree' | 'B' | 'B+D'>(
		(deal?.wagertype as 'Wagerfree' | 'B' | 'B+D') ?? 'Wagerfree'
	);

	let logourl = $state(deal?.logourl ?? '');
	let reflink = $state(deal?.reflink ?? '');
	let position = $state<number | ''>(typeof deal?.position === 'number' ? deal.position : '');

	let features = $state<string[]>(Array.isArray(deal?.features) ? [...deal.features] : []);
	let featureDraft = $state('');

	let payments = $state<string[]>(Array.isArray(deal?.payments) ? [...deal.payments] : []);
	let information = $state(deal?.information ?? '');
...
<svelte:head>
	<title>Edit Deal - Money4Duo</title>
</svelte:head>
	...
	<div class="mx-auto w-full max-w-3xl">
		...
		>
			...
			<div class="mb-6 space-y-4">
				...
				<label class="block">
					<span class="mb-1 block text-sm text-white/80">Position</span>
					<input
						type="number"
						min="0"
			...
			<div class="flex flex-col items-center justify-between gap-3 sm:flex-row">
				<p class="text-xs text-white/60">Save your changes.</p>
